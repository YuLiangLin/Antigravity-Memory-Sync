    ...
    foreach ($t in $targets) {
        ...
        if (-not $folderId) {
            ...
            try {
                ...
                if (-not $config.google_drive.folder_ids) {
                    $config.google_drive | Add-Member -NotePropertyName 'folder_ids' -NotePropertyValue @{} -Force
                }
                $config.google_drive.folder_ids | Add-Member -NotePropertyName $t -NotePropertyValue $folderId -Force
                $config | ConvertTo-Json -Depth 4 | Set-Content -Path $configPath -Encoding UTF8
                Write-Host "  ✅ Created '$t/' (ID: $folderId)" -ForegroundColor Green
            }
            catch {
                Write-Host "  ⚠️  Failed to create folder for '$t' — skipping" -ForegroundColor Yellow
                continue
            }
        }
